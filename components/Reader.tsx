...
import { Volume2, Loader2, Play, Settings2, Rabbit, Turtle, Bookmark as BookmarkIcon, List, X, Trash2, AlertCircle, Pause, Square, Maximize2, Minimize2, Sun, Moon, Coffee, Mic2, Headphones, Check, Type, AlignLeft, MoveHorizontal } from 'lucide-react';
  ...
  const handleSpeak = async () => {
    ...
    try {
      ...
      switch (err.message) {
        ...
        case "API_KEY_MISSING": friendlyMessage = "Configuration de la clé API manquante."; break;
        case "API_KEY_INVALID": friendlyMessage = "Autorisation de la clé API refusée."; break;
      ...
      <div className="flex-1 flex flex-col relative overflow-hidden h-full">
        ...
        {!isZenMode && (
          <div className="absolute bottom-12 left-1/2 -translate-x-1/2 z-40 flex flex-col items-center gap-4 w-full max-w-[95%] sm:max-w-md animate-in slide-in-from-bottom-8 duration-1000">
            {ttsError && (
              <div className="bg-red-50/90 backdrop-blur-md border border-red-100 text-red-800 px-5 py-3 rounded-2xl shadow-2xl flex items-center gap-3 w-full animate-in zoom-in-95">
                <AlertCircle size={20} className="flex-shrink-0 text-red-500" />
                <span className="text-xs font-bold leading-tight">{ttsError}</span>
              </div>
            )}
            ...
            {showSettings && (
              <div className="bg-white/95 backdrop-blur-xl border border-slate-200/50 shadow-2xl rounded-[2.5rem] p-8 mb-2 w-full animate-in fade-in slide-in-from-bottom-4 duration-300 max-h-[70vh] overflow-y-auto">
                <div className="space-y-8">
                  <div>
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-4">Ambiance visuelle</p>
